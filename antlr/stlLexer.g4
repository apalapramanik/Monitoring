...
DomainTypeFloat	: 'float' ;

DomainTypeLong	: 'long' ;

DomainTypeComplex	: 'complex' ;
...
AlwaysOperator	: 'always' | 'G' ;

EventuallyOperator	: 'eventually' | 'F' ;

UntilOperator	: 'until' | 'U' ;

UnlessOperator	: 'unless' | 'W' ;

HistoricallyOperator	: 'historically' | 'H' ;

OnceOperator	: 'once' | 'O' ;

SinceOperator	: 'since' | 'S' ;

NextOperator	: 'next' | 'X' ;

PreviousOperator	: 'prev' | 'Y' ;
...
fragment DecimalRealLiteral	: Digits '.' Digits? ExponentPart?	| '.' Digits ExponentPart?	| Digits ExponentPart	;

fragment ExponentPart	: ExponentIndicator SignedInteger ;

fragment ExponentIndicator	: [eE] ;

fragment SignedInteger	: Sign? Digit+ ;

fragment Sign	: [+-] ;

Identifier	: ((IdentifierStart)(IdentifierPart)*) ;

fragment IdentifierStart	: (LetterOrUnderscore | '$') ;
